...
exports.reportadoQuestion = async(agent) => {
    try {
        ...
        if (reportado == 'SI' && datos.ciudad == 'Santa Marta') {
            ...
            agent.add('Con cupo BRILLA los requisitos serían:');
            agent.add('1) Las dos últimas facturas originales pagadas (no duplicados)');
            agent.add('2) Tu cédula');
            agent.add('3) Y que el solicitante sea el titular de la factura.');
        ...
        console.log('Pregunta reportado', error);
    }
};
...
exports.estudioEnLinea = async(agent) => {
    try {
        ...
        if (action == 'estudio-yes') {
            ...
            console.warn('Ninguna acción');
        }
    } catch (error) {
        console.log('Error en estudio en linea:', error);
    }
};
...
exports.entidadReporta = async(agent) => {
    try {
        ...
        console.log('Error entidad que reporta', error);
    }
};
...
exports.valorAdeudo = async(agent) => {
    try {
        ...
        console.log('Error en valor adeudado', error);
    }
};
...
exports.cumpleRequisitosCupobrilla = async(agent) => {
    try {
        ...
        console.log('Error en validar requisitos', error);
    }
};